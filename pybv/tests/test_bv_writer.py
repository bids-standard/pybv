...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "When array, events must have 2 or 3 columns, but got: 4",
        ),
        (
            np.array([i for i in "abcd"]).reshape(2, -1),
            "When array, all entries in events must be int",
        ),
        (events_array, ""),
        (None, ""),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [{"onset": 1, "description": 2}, np.ones(12)],
            "events must be a list of dict",
        ),
        ([{"onset": "1", "description": 2}], "events: `onset` must be int"),
        (
            [{"onset": -1, "description": 2}],
            r"events: at least one onset sample is not in range of data \(0-4999\)",
        ),
        (
            [{"onset": 100, "description": 1, "duration": -1}],
            "events: at least one duration is negative.",
        ),
        (
...
@pytest.mark.parametrize(
    ...
    [("20000101120000000000", ref_ch_name), (datetime(2000, 1, 1, 12, 0, 0, 0), None)],
)
def test_write_read_cycle(tmpdir, meas_date, ref_ch_names):
    ...
    with pytest.warns(UserWarning, match="Encountered small Greek letter mu"):
        write_brainvision(
            ...
            folder_out=tmpdir,
            events=events_array,
            resolution=np.power(10.0, -np.arange(10)),
            unit="μV",
            meas_date=meas_date,
            overwrite=True,
        )
    ...
    assert raw_written.info["meas_date"] == datetime(
        2000, 1, 1, 12, 0, 0, 0, tzinfo=timezone.utc
    )
...
def test_write_multiple_units(tmpdir, unit):
    ...
    assert all(
        [orig_units[idx] == unit.replace("u", "µ") for idx in range(n_chans // 2)]
    )
    assert all(
        [
...
@pytest.mark.parametrize(
    "ref_ch_names", (None, ref_ch_name, [ref_ch_name] * n_chans, "foobar")
)
